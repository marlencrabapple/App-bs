...
field $out = [];
field $deps = [];
field $res : accessor = []; # Might supposed to be a method
...
method run : common ($argv, %runopts) {
    ...
    my $self = $class->new(
        dest  => delete $runopts{cliopt_dest},
        %runopts
    );

    $self->pkg_base_names( $self->queue );

    BS::Common::dmsg({ %runopts{qw(queue cliopt_dest res)} });
