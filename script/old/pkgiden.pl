...
method run ( $argv = \@ARGV ) {
    ...
    BS::Common::dmsg( { cmd => [ qw(expac), $expac_op, $expac_fmt, '...' ] } );

    foreach my $arg (@$queue) {
        my (@out);    #, @err);
        BS::Common::dmsg(
            { cmd => [ qw(expac), $expac_op, $expac_fmt, '...' ] } );
        my $run3err = run3(
            [ qw(expac), $expac_op, $expac_fmt, $arg ],
            \undef,
            sub ($line) {
                chomp $line;
    ...
    #warn join "\n", @$err if scalar @$err;
    BS::Common::dmsg( { self => $self, err => $err } );
}
