...
method fetch :common ($pkgstr, %args) {
  ...
  elsif (any { $args{repo}->{name} eq $_ && $args{repo}->{base_uri} } keys $args{repo_enabled}->%*) {
    $res = $class->bsx([qw(git clone --bare $)])
  }
  else {
    $res = $class->bsx([qw(pkgctl repo clone --protocol=https), $$pkgres{base}])
  }

  carp $res->out;

  $class->bsx([qw(git clone),  "$srccache/$$pkgres{base}", "$workdir/$$pkgres{base}"]);
