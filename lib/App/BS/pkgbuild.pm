...
class App::BS::pkgbuild : isa(App::BS::CLI) : does(App::BS::CLI::Util);
    ...
    $build_dirty = !$stay_fresh )
{
    ...
    state @bs_args = (
        ...
        "C,u,n$clean_chroot",
        '--margs',
        "L,A,s,f,i$makepkg_cleanall",
        '--makepkg_args',
        $self->env->{makepkg_conf},
        '--pacman-conf',
        $self->env->{pacman_conf},
...
use constant DEFAULT_REFS => (
    origin => \DEFAULT_BRANCHES,
